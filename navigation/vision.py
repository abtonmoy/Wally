...
class HybridNavigator:
    ...
    def calculate_navigation_speed_radius(self, angle, dist):
        ...
        if abs(angle) < 0.1:  # Small angle, go straight
            radius = float('inf')
        else:
        ...
        while True:
            try:
                ...
                if self.current_mode == self.MODE_GPS_NAVIGATION:
                    ...
                    # Use camera distance if available and lidar-confirmed, otherwise use lidar min distance
                ...
                if speed <= 0:
                    self.stop_robot()
                elif radius > 2.0:  # Essentially straight
                    self.send_command(Commands["drive_straight"], speed)
                else:
                    # Determine turn direction based on current mode
                    if self.current_mode == self.MODE_GPS_NAVIGATION:
                        # Use navigation error to determine turn direction
                        turn_radius = radius if nav_error >= 0 else -radius
                    else:
                        # Use gap angle to determine turn direction
